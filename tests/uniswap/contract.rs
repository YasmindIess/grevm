...
impl UniswapV3Factory {
    ...
    pub fn add_pool(
        ...
        self.pools.insert((token_0, token_1, U256::from(POOL_FEE)), pool_address);
        self
    }
    ...
    pub fn build(&self, address: Address) -> PlainAccount {
        let hex = UNISWAP_V3_FACTORY
            .trim()
            .replace("0b748751e6f8b1a38c9386a19d9f8966b3593a9e", &address.encode_hex());
        let bytecode = Bytecode::new_raw(Bytes::from_hex(hex).unwrap());
        ...
        for ((token_0, token_1, pool_fee), pool_address) in self.pools.iter() {
            store.set(
                from_indices(5, &[from_address(*token_0), from_address(*token_1), *pool_fee]),
                from_address(*pool_address),
            );
            store.set(
                from_indices(5, &[from_address(*token_1), from_address(*token_0), *pool_fee]),
                from_address(*pool_address),
            );
        }
...
impl UniswapV3Pool {
    ...
    pub fn build(&self, address: Address) -> PlainAccount {
        let hex = UNISWAP_V3_POOL
            .trim()
            .replace("261d8c5e9742e6f7f1076fa1f560894524e19cad", &self.token_0.encode_hex())
            .replace("ce3478a9e0167a6bc5716dc39dbbbfac38f27623", &self.token_1.encode_hex())
            .replace("cba6b9a951749b8735c603e7ffc5151849248772", &self.factory.encode_hex())
            .replace("d495d5e5cab2567777fff988c4fcd71328b17c9d", &address.encode_hex());
        let bytecode = Bytecode::new_raw(Bytes::from_hex(hex).unwrap());

        let mut store = StorageBuilder::new();
        store
            .set(0, uint!(0x0001000001000100000000000000000000000001000000000000000000000000_U256));
        store.set(1, 0);
        store.set(2, 0);
        store.set(3, 0);
        ...
        store
            .set(8, uint!(0x0100000000000000000000000000000000000000000000000000000000000001_U256));
...
impl SwapRouter {
    pub fn new(weth9: Address, factory: Address, pool_init_code_hash: B256) -> Self {
        Self { weth9, factory, pool_init_code_hash }
    }
    ...
    pub fn build(&self) -> PlainAccount {
        let hex = SWAP_ROUTER
            .trim()
            .replace("6509f2a854ba7441039fce3b959d5badd2ffcfcd", &self.weth9.encode_hex())
            .replace("d787a42ee3ac477c46dd6c912e7af795d44453d5", &self.factory.encode_hex())
            .replace(
        ...
        let mut store = StorageBuilder::new();
        store
            .set(0, uint!(0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff_U256));
...
impl SingleSwap {
    pub fn new(swap_router: Address, token_0: Address, token_1: Address) -> Self {
        Self { swap_router, token_0, token_1 }
    }
    ...
    pub fn build(&self) -> PlainAccount {
        let hex = SINGLE_SWAP
            .trim()
            .replace("e7cfcccb38ce07ba9d8d13431afe8cf6172de031", &self.swap_router.encode_hex());
        let bytecode = Bytecode::new_raw(Bytes::from_hex(hex).unwrap());
